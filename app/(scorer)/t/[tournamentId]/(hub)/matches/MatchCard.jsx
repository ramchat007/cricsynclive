...
export default function MatchCard({ match, isAdmin, deleteMatch }) {
  ...
  return (
    <div
      key={match.id}
      className="bg-[var(--surface-1)] rounded-3xl p-4 md:p-5 border border-[var(--border-1)] relative group transition-all duration-300 hover:border-[var(--accent)]/40 hover:shadow-xl flex flex-col justify-between gap-4 overflow-hidden h-full"
    >
      ...
      <div className="flex items-center justify-between gap-2 border-b border-[var(--border-1)]/60 pb-3">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          {/* Match Number Chip */}
          <span className="text-[10px] font-black uppercase tracking-wider text-[var(--accent)] bg-[var(--accent)]/10 border border-[var(--accent)]/20 px-2.5 py-0.5 rounded-lg shrink-0">
            M#{match.match_no || "TBA"}
          </span>

          {/* Stage Chip */}
          {match.stage && (
            <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] bg-[var(--surface-2)] border border-[var(--border-1)] px-2.5 py-0.5 rounded-lg truncate max-w-[120px] md:max-w-[180px]">
              {match.stage}
            </span>
          )}

          {/* Overs Badge */}
          <span className="text-[10px] font-bold text-[var(--text-muted)] bg-[var(--surface-2)] px-2 py-0.5 rounded-md shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {match.status === "live" ? (
            <span className="flex items-center gap-1.5 px-2.5 py-0.5 bg-red-500/10 border border-red-500/30 rounded-full text-[10px] font-black text-red-500 tracking-widest animate-pulse">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500" />
              LIVE
            </span>
          ) : match.status === "completed" ? (
            <span className="px-2.5 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-[10px] font-bold text-emerald-500 tracking-wider">
              ENDED
            </span>
          ) : (
            <span className="px-2.5 py-0.5 bg-[var(--surface-2)] border border-[var(--border-1)] rounded-full text-[10px] font-bold text-[var(--text-muted)] tracking-wider">
              UPCOMING
            </span>
          )}
      ...
      <div className="bg-[var(--surface-2)]/40 border border-[var(--border-1)]/80 rounded-2xl p-3 md:p-4 flex flex-col gap-3">
        {/* TEAM 1 ROW */}
        <div
