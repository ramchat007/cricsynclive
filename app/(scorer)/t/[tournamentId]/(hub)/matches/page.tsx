...
export default function MatchesPage({
  ...
  // --- NEW: V1 WIZARD ENGINE STATES ---
  const [wizardMode, setWizardMode] = useState("round_robin"); // 'round_robin', 'groups_knockout', 'pure_knockout'
  ...
  const [wizardTeamsCount, setWizardTeamsCount] = useState(8); // For pure knockout mode
  ...
  // Helper to find the next available Match Number
  ...
  const handleCreateSingleMatch = async () => {
    ...
    const { error } = await supabase.from("matches").insert({
      ...
      stage: stage,
      status: "scheduled",
      match_no: nextNo, // Assigned Match Number
    });
  ...
  // --- 2. TIME CALCULATION ENGINE (WITH MATCH NUMBERS) ---
  const generateTimeSlots = (generatedMatches: any[]) => {
    ...
    for (let i = 0; i < generatedMatches.length; i++) {
      ...
      finalizedMatches.push({
        ...generatedMatches[i],
        match_no: currentMatchNo++, // Auto-assigns sequentially
  ...
  // --- 3. V1 WIZARD ENGINE TRANSLATED TO SUPABASE ---
          ...
          {scheduleMode === "single" ? (
            ...
            <div className="space-y-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <div className="pt-6 border-t border-slate-100">
                <h3 className="font-black uppercase tracking-widest text-slate-900 mb-4">
                  Tournament Structure
                </h3>
                <div className="flex gap-2 mb-6 bg-slate-100 p-1 rounded-xl">
                  <button
                    onClick={() => setWizardMode("round_robin")}
                    className={`flex-1 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all ${wizardMode === "round_robin" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"}`}
                  >
                    Round Robin + KO
                  </button>
                  <button
                    onClick={() => setWizardMode("groups_knockout")}
                    className={`flex-1 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all ${wizardMode === "groups_knockout" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"}`}
                  >
                    Groups + KO
                  </button>
                  <button
                    onClick={() => setWizardMode("pure_knockout")}
                    className={`flex-1 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all ${wizardMode === "pure_knockout" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"}`}
                  >
                    Pure Knockout
                  </button>
                </div>
              ...
              {/* POOL SELECTION (Only for modes that use actual teams) */}
              {wizardMode !== "pure_knockout" && (
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-center px-4 md:px-8 py-6 w-full relative mt-4 md:mt-0">
                ...
                <div className="absolute -top-1 md:top-3 left-1/2 -translate-x-1/2 flex items-center gap-2 whitespace-nowrap">
                  <span className="text-[9px] font-black uppercase tracking-widest text-teal-600 bg-teal-50 border border-teal-100 px-3 py-1 rounded-full shadow-sm">
                    Match {match.match_no || "TBA"}
                  </span>
                  <span className="text-[9px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-50 border border-indigo-100 px-3 py-1 rounded-full shadow-sm">
                    {match.stage}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center gap-3 w-[40%]">
                    <div
                      className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-slate-50 bg-contain bg-center bg-no-repeat border border-slate-100 shadow-inner p-2 flex items-center justify-center text-slate-300 font-bold text-[10px] text-center"
                      style={{
