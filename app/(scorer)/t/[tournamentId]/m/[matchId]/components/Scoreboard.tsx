...
export default function Scoreboard({
  ...
  <div className="bg-[var(--surface-1)] p-2 sm:p-4 rounded-xl sm:rounded-2xl border border-[var(--border-1)] shadow-sm flex flex-col md:flex-row items-center justify-between gap-2 transition-colors">
    ...
    <div className="flex items-center justify-end gap-1.5 sm:gap-2 text-xs sm:text-lg font-black uppercase tracking-tight truncate min-w-0">
      ...
      >
        {team1Name} <span className="hidden sm:inline">{!isCompleted && isTeam1Batting && "🏏"}</span>
      </span>
      ...
      <span
        className={`truncate transition-colors ${!isCompleted && !isTeam1Batting ? "text-[var(--accent)]" : "text-[var(--foreground)]"}`}
      >
        <span className="hidden sm:inline">{!isCompleted && !isTeam1Batting && "🏏 "}</span>{team2Name}
      </span>
    </div>
  </div>
  ...
  >
    ...
    {isSecondInnings && (
      <div className="bg-[var(--surface-2)] p-3 sm:p-6 rounded-xl sm:rounded-[2rem] border border-[var(--border-1)] shadow-inner flex flex-row md:flex-col justify-between md:justify-center items-center text-left md:text-center md:opacity-80 relative gap-2 md:gap-0">
        
        {/* Mobile Left / Desktop Top */}
        <div>
          <p className="text-[9px] sm:text-xs font-black text-[var(--text-muted)] uppercase tracking-widest md:mb-1">
            1st Inn <span className="hidden md:inline">• {isTeam1Batting ? team2Name : team1Name}</span>
          </p>
          <div className="md:hidden mt-0.5 text-[9px] font-bold text-[var(--text-muted)] uppercase">
            Overs: <span className="text-[var(--foreground)]">{inn1Overs}</span>
          </div>
        </div>

        {/* Mobile Right / Desktop Bottom */}
        <div className="flex flex-col items-end md:items-center">
          <h2 className="text-2xl sm:text-6xl font-black text-[var(--foreground)] leading-none tracking-tighter">
            {targetScore ? targetScore - 1 : "0"}
            <span className="text-base sm:text-4xl text-[var(--text-muted)] opacity-70">
              /{inn1Wickets}
            </span>
          </h2>
          <div className="hidden md:block mt-2 text-[12px] font-bold text-[var(--text-muted)] bg-[var(--surface-1)] px-2 py-1 rounded-md border border-[var(--border-1)] uppercase">
            Overs: <span className="text-[var(--foreground)] ml-1">{inn1Overs}</span>
          </div>
        </div>
      </div>
    )}
    ...
    {/* ACTIVE / LIVE SCORE */}
    <div
      className={`${!isSecondInnings ? "md:col-span-2" : ""} bg-[var(--surface-1)] p-3 sm:p-6 rounded-xl sm:rounded-3xl border ${isCompleted ? "border-[var(--border-1)]" : "border-[var(--accent)]/50 shadow-[0_0_15px_rgba(var(--accent-rgb),0.1)]"} flex flex-col justify-center items-center text-center relative overflow-hidden`}
    >
      <p className="text-[9px] sm:text-xs font-black text-[var(--text-muted)] uppercase tracking-widest mb-1 relative z-10">
        {isSecondInnings ? "2nd Innings" : "1st Innings"} <span className="hidden sm:inline">• {battingTeam?.short_name || battingTeam?.name}</span>
      </p>

      <h1
        className={`text-5xl sm:text-7xl lg:text-8xl font-black text-[var(--foreground)] leading-none tracking-tighter relative z-10 ${isCompleted ? "opacity-80" : ""}`}
      >
        {currentScore}
        <span className="text-2xl sm:text-4xl text-[var(--text-muted)] opacity-70">
          /{currentWickets}
        </span>
      </h1>

      {/* Mobile-Only Unified Metrics */}
      <div className="mt-3 sm:mt-4 flex flex-wrap items-center justify-center gap-1.5 relative z-10 md:hidden">
        <div className="flex items-center text-[10px] font-bold text-[var(--text-muted)] bg-[var(--surface-2)] px-2 py-1 rounded-lg border border-[var(--border-1)] uppercase tracking-widest">
          Overs:{" "}
          ...
          {isAuthorized && !isCompleted && (
            <button
              ...
              className="ml-1 text-[var(--accent)] hover:scale-110 active:scale-95 p-1 -m-1"
            >
              <Settings size={12} />
            </button>
          )}
        </div>
        <div className="text-[10px] font-bold text-[var(--text-muted)] bg-[var(--surface-2)] px-2 py-1 rounded-lg border border-[var(--border-1)] uppercase tracking-widest">
          CRR:{" "}
      ...
      {!isCompleted && tapeDeliveries?.length > 0 && (
        <div className="mt-3 sm:mt-4 pt-2 sm:pt-4 border-t border-[var(--border-1)] w-full flex flex-col items-center relative z-10">
          <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 w-full justify-center">
            <span className="text-[8px] sm:text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest">
              {tapeLabel}
            </span>
            <div className="flex gap-1 sm:gap-1.5 overflow-x-auto hide-scrollbar max-w-full sm:max-w-md px-1 pb-1">
              {tapeDeliveries.map((d: any, idx: number) => {
                ...
                return (
                  <span
                    key={d.id || idx}
                    className={`flex items-center justify-center min-w-[26px] h-6 sm:min-w-[32px] sm:h-8 px-1 rounded-full text-[10px] sm:text-xs font-black shadow-sm shrink-0 ${
                      isWicket
  ...
  <div className="text-center text-[10px] sm:text-xs font-bold text-[var(--text-muted)] mt-1 sm:mt-0">
    Extras:{" "}
