...
export default function PlayerSpotlight({
  ...
  useEffect(() => {
    ...
    const fetchSpotlightData = async () => {
      ...
      if (
        ...
        const balls = faced.filter((d) => d.extras_type !== "wd").length;
        ...
        const runsGiven = bowled.reduce(
          (acc, d) =>
            acc + (Number(d.runs_off_bat) || 0) + (Number(d.extras_runs) || 0),
          0,
        );
        const wickets = bowled.filter((d) => d.is_wicket).length; // Add specific wicket types if needed
  ...
  return (
    <div
      className="flex bg-slate-900 border-[3px] border-cyan-500 rounded-2xl shadow-[0_0_30px_rgba(6,182,212,0.4)] min-w-[450px] overflow-hidden"
      style={{
        animation:
    ...
    >
      {/* Player Photo / Avatar */}
      <div className="w-[140px] bg-gradient-to-t from-slate-950 to-slate-800 flex items-end justify-center relative border-r border-slate-700">
        {player.profile_url ? (
          <img
      ...
      <div className="flex-1 p-5 flex flex-col justify-center">
        <h3 className="text-cyan-400 font-black tracking-[0.3em] uppercase text-[10px] flex items-center gap-1 mb-1">
          <Target size={12} /> Player Spotlight
        </h3>
        <h2 className="text-white font-black text-2xl uppercase tracking-tight leading-none mb-1">
          {player.full_name}
        </h2>
        ...
        {stats.type === "batting" && (
          <div className="grid grid-cols-4 gap-2 bg-slate-950 p-3 rounded-xl border border-slate-800">
            ...
            <div className="text-center">
              ...
              <div className="text-xl text-cyan-400 font-black">
