...
export default function RemoteControl({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  const startSmoothZoom = (direction: number) => {
    const min = camCapabilities?.zoom?.min || 1; const max = camCapabilities?.zoom?.max || 10;
    const stepSpeed = (max - min) * 0.05; 
    zoomIntervalRef.current = setInterval(() => {
      let currentZ = Number(remoteZoomRef.current) || 1;
      let newZoom = Math.min(Math.max(currentZ + stepSpeed * direction, min), max); 
      setRemoteZoom(newZoom); sendCommand("zoom", newZoom); lastZoomTime.current = Date.now();
    }, 150);
  };
  ...
  const handleExposureChange = (e: React.ChangeEvent<HTMLInputElement>) => { const val = Number(e.target.value); setRemoteExposure(val); sendCommand("exposure", val); lastExposureTime.current = Date.now(); };
      ...
      <div className="w-full max-w-3xl bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-2xl my-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mt-6">
          <div className="md:col-span-4 space-y-4 flex flex-col">
            <button onClick={toggleRemoteMute} className={`p-4 rounded-xl flex flex-col items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest transition-all border shadow-lg ${remoteMuted ? "bg-red-500/20 text-red-500 border-red-500/50" : "bg-gray-950 text-emerald-500 border-gray-800 hover:bg-gray-900"}`}>{remoteMuted ? <MicOff size={24} /> : <Mic size={24} />} {remoteMuted ? "Muted" : "Mic Active"}</button>
            <button onClick={toggleOledSleep} className={`p-4 rounded-xl flex flex-col items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest transition-all border shadow-lg ${remoteOled ? "bg-indigo-600 text-white border-indigo-500 shadow-[0_0_15px_rgba(79,70,229,0.4)]" : "bg-gray-950 text-indigo-400 border-gray-800 hover:bg-gray-900"}`}>{remoteOled ? <Moon size={24} /> : <Sun size={24} />} {remoteOled ? "Screen Off" : "Screen On"}</button>
            {camCapabilities?.torch !== undefined && (<button onClick={toggleRemoteTorch} className={`w-full flex-1 p-4 rounded-xl flex flex-col items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest transition-all border shadow-lg ${remoteTorch ? "bg-amber-500 text-black border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.4)]" : "bg-gray-950 text-amber-500 border-gray-800 hover:bg-gray-900"}`}>{remoteTorch ? <Flashlight size={24} /> : <ZapOff size={24} />} {remoteTorch ? "Torch ON" : "Torch OFF"}</button>)}
          </div>
          ...
          <div className="md:col-span-8 space-y-4 flex flex-col">
            <div className="bg-gray-950 p-5 rounded-xl border border-gray-800 flex-1 flex flex-col shadow-lg relative overflow-hidden">
              ...
              <div className="flex items-center gap-6 mt-auto">
                <div className="flex flex-col gap-1 shrink-0 bg-gray-900 p-1 rounded-xl border border-gray-800">
                  <button onMouseDown={() => startSmoothZoom(1)} onMouseUp={stopSmoothZoom} onMouseLeave={stopSmoothZoom} onTouchStart={(e) => { e.preventDefault(); startSmoothZoom(1); }} onTouchEnd={(e) => { e.preventDefault(); stopSmoothZoom(); }} className="w-16 h-14 bg-gray-800 hover:bg-gray-700 active:bg-cyan-600 rounded-t-lg flex items-center justify-center text-white transition-colors touch-none select-none"><Plus size={24} strokeWidth={3} /></button>
                  <button onMouseDown={() => startSmoothZoom(-1)} onMouseUp={stopSmoothZoom} onMouseLeave={stopSmoothZoom} onTouchStart={(e) => { e.preventDefault(); startSmoothZoom(-1); }} onTouchEnd={(e) => { e.preventDefault(); stopSmoothZoom(); }} className="w-16 h-14 bg-gray-800 hover:bg-gray-700 active:bg-cyan-600 rounded-b-lg flex items-center justify-center text-white transition-colors touch-none select-none"><Minus size={24} strokeWidth={3} /></button>
                </div>
                <div className="flex-1 flex flex-col gap-4">
                  <div className="flex justify-between items-center"><label className="text-xs font-black uppercase tracking-widest text-gray-500 flex items-center gap-1.5"><ZoomIn size={16} className="text-cyan-500" /> Zoom Level</label><span className="text-cyan-500 font-mono font-black text-base bg-cyan-500/10 px-3 py-1 rounded">{mappedDisplayZoom}x</span></div>
            ...
            <div className="bg-gray-950 p-5 rounded-xl border border-gray-800 shadow-lg">
              <div className="flex justify-between items-center mb-4"><label className="text-xs font-black uppercase tracking-widest text-gray-500 flex items-center gap-1.5"><Sun size={16} className="text-amber-500" /> Exposure (EV)</label><span className="text-amber-500 font-mono font-black text-base bg-amber-500/10 px-3 py-1 rounded">{Number(remoteExposure) > 0 ? "+" : ""}{Number(remoteExposure).toFixed(1)}</span></div>
              <div className="flex items-center gap-4"><span className="text-sm font-black text-gray-600">-</span><input type="range" min={rawExpMin} max={rawExpMax} step={camCapabilities?.exposure?.step || 0.1} value={Number(remoteExposure) || 0} onChange={handleExposureChange} className="flex-1 accent-amber-500 h-3 bg-gray-800 rounded-lg appearance-none cursor-pointer" /><span className="text-sm font-black text-gray-600">+</span></div>
            </div>
          </div>
        </div>

        <button onClick={handleKillStream} className="w-full mt-6 py-4 bg-red-600/10 hover:bg-red-600/20 text-red-500 border border-red-500/30 rounded-xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 active:scale-95 shadow-lg"><Power size={16} /> Emergency Kill Stream</button>
      </div>
    </div>
  );
}
